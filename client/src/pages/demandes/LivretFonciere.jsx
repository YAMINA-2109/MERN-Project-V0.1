...
const LivretFonciere = () => {
    ...
    const handlRefusedClick = ()=>{
     ...
     const handleRefusedFormSubmit = async (values) => {
      ...
      try {
        ...
        const response = await axios.post('http://localhost:3000/api/demandes/response/refusedLivret', {
    ...
    const handleFormSubmit = async (values) => {
      ...
      try {
        // Envoyer les données du formulaire au serveur
        const response = await axios.post('http://localhost:3000/api/demandes/Livretresponse', {
          reponses: values.comment, clientId: selectedDemande.clientId, demandeId: selectedDemande._id
        });
        if (response.data.success) {
          // Réponse réussie, fermez la modale
  ...
  const getdata = async ()=>{
      ...
      const res = await instance.post('/api/demandes/Livrets', {},{
        headers: {
            Cookie: Object.entries(cookies).map(([key, value]) => `${key}=${value}`).join('; ')
        }
      })
      setData(res.data.livretFonciers)
      console.log(res);
  }
  useEffect(() => {
    getdata()
